...
skip 2
DPEchoes: ;24-2B channels with echo active
skip 8
DPLegato: ;2c legato flag
skip 1
DPatLength: ;2C-xx measure total pattern lengths for channel 0
skip 128
...
-	dec y
	...
	mov ReadTrackX,#$07 ;read in backwards order
...
KonvertReadPattern:
	...
	jmp KonvertReadPattern
++	dec ReadTrackX
	cmp ReadTrackX,#$00 ;do not read more than 8 channels
	bpl +
-	nop
	bra -
+	cmp DPSubFlag,#$00
...
VoiceNoteEvent: ;01-D5
	...
	mov a,(ReadSeq)+y
	bmi +++
	cmp a,#$40 ;00-3F, quantization (w/velocity) 40-7F skip to velocity
	bpl ++
	mov x,a
	mov a,PresetISParam3+x
	lsr a
	and a,#$f0
	mov DPQuant,a
	or a,DPVelo
	...
	mov a,(ReadSeq)+y
	bmi +++
++	and a,#$3f ;write 00-7F param 3 as velocity
	mov x,a
	mov a,PresetISParam3+x
	and a,#$f0
	xcn a
...
+	cmp a,#$c9
	...
	bra ++
+	and a,#$7f
	cmp a,#$56 ;jump to voice command
	bmi +
	eor a,#$80
	jmp VoiceCommandRun
...
PresetISParam3:
	db $00,$0c,$19,$26,$33,$3f,$4c,$59,$66,$72,$75,$77,$70,$7c,$7f,$82
	db $84,$87,$89,$8c,$8e,$91,$93,$96,$99,$9b,$9e,$a0,$a3,$a5,$a8,$aa
	db $ad,$af,$b2,$b5,$b7,$ba,$bc,$bf,$c1,$c4,$c6,$c9,$cc,$ce,$d1,$d3
	db $d6,$d8,$db,$dd,$e0,$e2,$e5,$e8,$ea,$ed,$ef,$f2,$f4,$f7,$f9,$fc


VoiceCommandRun: ;D6-FF
	mov DParamSize,#$00
	setc
	sbc a,#$d6
	mov x,a
...
PresetVCMD: ;N-SPC to SMW VCMD conversion table [$DA-$FF]
	...
	db $DF ;da vibrato off
	db $00 ;db song volume
	db $E1 ;dc song volume fade
	db $00 ;dd tempo

	db $E3 ;de tempo fade
	db $E4 ;df global transposition
	db $00 ;e0 channel transposition
	db $E5 ;e1 tremolo on

	db $FD ;e2 tremolo off
	db $00 ;e3 volume
	db $E8 ;e4 volume fade
	db $00 ;e5 subroutine

	db $EA ;e6 vibrato fade
	db $EB ;e7 pitch env to
	db $EC ;e8 pitch env from
	db $FE ;e9 pitch enb disable

	db $EE ;ea fine detune
	db $EF ;eb echo p1
	db $F0 ;ec echo off
	db $F1 ;ed echo p2

	db $F2 ;ee echo vol fade
	db $DD ;ef pitch slide
	db $00 ;f0 percussion base
	db $00 ;f1 echo on
	...
	db $00 ;fa voice definer by amounts x 4 (skip)
	db $DA ;fb instrument
	db $00 ;fc adsr
	db $00 ;fd gain
...
VCMDISVoiceDef:
	inc y
	mov a,(ReadSeq)+y ;skip voice table (four bytes per entry)
	bmi +
	asl a
	asl a
	inc a
	mov y,a
-	jmp FinishCom
+	mov y,#$07 ;skip instrument overwrite (8 per entry)
	bra -
...
--	mov a,DPStack
	...
	bpl --
	inc y
	mov a,(ReadSeq)+y ;echo volume left
	call RoutineWriteHex
	inc y
	mov a,(ReadSeq)+y ;echo volume right
	call RoutineWriteHex
	jmp FinishCom
...
++	cmp DPSubFlag,#$00 ;measure length of current subroutine
	...
	inc DPatSubtime+1
+	mov DPatSubtime,a
++	cmp ReadTrackX,#$07 ;measure initial pattern lengths on track 7 per phrase
	bne ++
	mov a,DPatPhrase
	asl a
	mov x,a
